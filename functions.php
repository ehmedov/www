...
function talk($to,$text,$sender)
{
	...
	fwrite ($fopen_chat,"::".time()."::".$to."::#990000::".$text."::".$sender["room"]."::".$sender["city_game"]."::\n");
	fclose ($fopen_chat);
}	
//-------------------------------------------------
...
/*=====================testBlock=================================*/
function testBlok($who)
{
	$db = mysql_fetch_array(mysql_query("SELECT blok FROM users WHERE login='".$who."'"));
    if($db["blok"]!=0)
    {
...
/*============ПОКАЗАТЬ ЗВЕРЯ===========================*/
function showZver($zver_id)
{	
	$item=array();
	$descr=array();
	$item_view=array();
	$item['amulet']="amulet/amulet0.gif";
	$item['naruchi']="naruchi/naruchi0.gif";
	$item['hand_r']="wp/weapon0.gif";
	$item['armour']="armour/armour0.gif";
	$item['rubaxa']="armour/armour0.gif";
	$item['plash']="armour/armour0.gif";
	$item['poyas']="belt/belt0.gif";
	$item['helmet']="helmet/helmet0.gif";
	$item['mask']="helmet/helmet0.gif";
	$item['masdrikon']="helmet/helmet0.gif";
	$item['perchi']="arm/arm0.gif";
	$item['rukavic']="arm/arm0.gif";
	$item['ring1']="ring/ring0.gif";
	$item['ring2']="ring/ring0.gif";
	$item['ring3']="ring/ring0.gif";
	$item['pants']="pants/pants0.gif";
	$item['hand_l']="wp/wp0.gif";
	$item['boots']="boots/boot0.gif";
	...
	$sql=mysql_query ("SELECT * FROM zver WHERE id=$zver_id");
	...
	$udacha=$dat["udacha"];
	$power=$dat["power"];
	$intellekt=$dat["intellekt"];
	$vospriyatie=$dat["vospriyatie"];
	

	$inff="<b>Информация о ".$name."</b><br>• Сила: ".$sila."<br>• Ловкость: ".$lovkost."<br>• Удача: ".$udacha."<br>• Выносливость: ".$power."<br>• Интеллект: ".$intellekt."<br>• Восприятие: ".$vospriyatie;
	echo "<table border=0 width=220 cellpadding=0 cellspacing=0 >
	 <tr>
	    <td width=60 height=220 valign=top>
			<script>";
				echo "
				view_item('".$item['helmet']."',60,60,'".$descr['helmet']."','".$style['helmet']."','".$item_view['helmet']."');\n
				view_item('".$item['masdrikon']."',60,60,'".$descr['masdrikon']."','".$style['masdrikon']."','".$item_view['masdrikon']."');\n
				view_item('".$item['naruchi']."',60,30,'".$descr['naruchi']."','".$style['naruchi']."','".$item_view['naruchi']."');\n
				view_item('".$item['hand_r']."',60,60,'".$descr['hand_r']."','".$style['hand_r']."','".$item_view['hand_r']."');\n
				view_item('".$item['armour']."',60,80,'".$descr['armour']."','".$style['armour']."','".$item_view['armour']."');\n
				view_item('".$item['poyas']."',60,30,'".$descr['poyas']."','".$style['poyas']."','".$item_view['poyas']."');\n
			</script>
		</td>
		<td width=140 height=220 valign=top>
			<script>
				view_item('".$obraz."',140,220,'".$inff."');\n
				view_item('wp/banner.gif',140,40,'WWW.MEYDAN.AZ');\n
			</script>
		</td>
		<td width=60 height=220 valign=top>
			<script>
				view_item('".$item['amulet']."',60,20,'".$descr['amulet']."','".$style['amulet']."','".$item_view['amulet']."');\n
				view_item('".$item['ring1']."',20,20,'".$descr['ring1']."','".$style['ring1']."','".$item_view['ring1']."');
				view_item('".$item['ring2']."',20,20,'".$descr['ring2']."','".$style['ring2']."','".$item_view['ring2']."');
				view_item('".$item['ring3']."',20,20,'".$descr['ring3']."','".$style['ring3']."','".$item_view['ring3']."');
				view_item('".$item['perchi']."',60,40,'".$descr['perchi']."','".$style['perchi']."','".$item_view['perchi']."');\n
				view_item('".$item['rukavic']."',60,40,'".$descr['rukavic']."','".$style['rukavic']."','".$item_view['rukavic']."');\n
				view_item('".$item['hand_l']."',60,60,'".$descr['hand_l']."','".$style['hand_l']."','".$item_view['hand_l']."');\n
				view_item('".$item['pants']."',60,80,'".$descr['pants']."','".$style['pants']."','".$item_view['pants']."');\n
				view_item('".$item['boots']."',60,40,'".$descr['boots']."','".$style['boots']."','".$item_view['boots']."');\n
			</script>
		</td>
	</tr>
	</table>";
...
{	
	...
	while ($dat=mysql_fetch_array($sql))
	{
		...
		{
			if($myinfo['battle']==0)unWear($myinfo['login'],$itm);
		}
		else if ($myinfo['bs']!=1 && $dat["bs"]==1)
		{
			unWear($myinfo['login'],$itm);
		}
		else if ($del_time!="" && $del_time<time())
		{
			unWear($myinfo['login'],$itm);
			...
			if(mysql_affected_rows()>0)
			{
				...
				history($myinfo["login"],"Закончился срок аренды",$name,$myinfo["remote_ip"],"showPlayer");
			}
		}
		...
		else
		{
			...
			$artimg="";
			$ordimg="";
			if($is_personal){$desc.="<img src=img/personal.gif border=0> ";}
			if($art==1){$desc.="<img src=img/artefakt.gif border=0> ";}
			if($art==2){$desc.="<img src=img/icon/artefakt.gif border=0> ";}
			if($podzemka){$desc.="<img src=img/icon/podzemka.gif border=0> ";}
			if($need_orden){$desc.="<img src=img/orden/$need_orden/0.gif border=0> ";}
			if($is_runa){$desc.="<img src=img/icon/runa.gif border=0> ";}
			$desc.="<b>$name ".($is_modified?" +$is_modified":"")." ($iznos/$iznos_all)</b>";
			if ($dat["add_xp"]>0){$desc.="<br>• <b style=color:green>Подогнано: ".$dat["add_xp"]." HP</b>";}
			if($gravirovka!=""){$desc.="<br>• Выгравирована надпись: ".$gravirovka;}
			if($add_vinoslivost){$desc.="<br>• Уровень ХП: +".($add_vinoslivost+$dat["add_xp"]);}
			if($add_mana){$desc.="<br>• Уровень маны: +$add_mana";}
			
			if($bron_head){$desc.="<br>• Броня головы: +$bron_head";}
			if($bron_corp){$desc.="<br>• Броня корпуса: +$bron_corp";}
			if($bron_poyas){$desc.="<br>• Броня пояса: +$bron_poyas";}
			if($bron_leg){$desc.="<br>• Броня ног: +$bron_leg";}
			
			if($min_attack){$desc.="<br>• Мин. урон: ".($min_attack+$is_modified);}
			if($max_attack){$desc.="<br>• Макс. урон: ".($max_attack+$is_modified);}
			if ($del_time!="") $desc.="<BR><b>Аренда: до ".(date('d.m.y H:i:s', $del_time))."</b>";
			if ($is_personal)	$desc.="<br><font color=brown>Личное оружие персонажа <b>$personal_owner</b></font>";
		}
		$descr[$slot]=$desc;
		$item[$slot]=$dat['img'];
		$item_unwear[$slot]=$itm;
		$item_view[$slot]=$obj_id;
		if ($is_personal || ($art==2 && ($slot=="hand_r"||$slot=="hand_l"))) $style[$slot]="class=ispersonal";else $style[$slot]="";
		$krit_iznos = ceil(($iznos_all/100)*80);
		if($iznos>=$krit_iznos && $iznos<$iznos_all)
        {
        	$style[$slot]="class=glow_pink";
		}
	}
	mysql_free_result($sql);
	 ...
	 <tr>
	    <td width=60 height=220 valign=top>
			<script>";
				...
				echo "
				view_item('items/".$item['naruchi']."',60,30,'".$descr['naruchi']."','".$style['naruchi']."','".$item_view['naruchi']."');\n
				view_item('items/".$item['hand_r']."',60,60,'".$descr['hand_r']."','".$style['hand_r']."','".$item_view['hand_r']."');\n";
				if ($item_unwear['plash'])
				{
					echo "view_item('items/".$item['plash']."',60,80,'".$descr['plash'].($item_unwear['armour']?"<hr color=#504F4C size=1 width=50%>".$descr['armour']:"").($item_unwear['rubaxa']?"<hr color=#504F4C size=1 width=50%>".$descr['rubaxa']:"")."','".$style['plash']."','".$item_view['plash']."');\n";
				}
				else if ($item_unwear['armour'])
				{
					echo "view_item('items/".$item['armour']."',60,80,'".$descr['armour'].($item_unwear['rubaxa']?"<hr color=#504F4C size=1 width=50%>".$descr['rubaxa']:"")."','".$style['armour']."','".$item_view['armour']."');\n";
				}
				else
				{
					echo "view_item('items/".$item['rubaxa']."',60,80,'".$descr['rubaxa']."','".$style['rubaxa']."','".$item_view['rubaxa']."');\n";
				}
				echo "view_item('items/".$item['poyas']."',60,30,'".$descr['poyas']."','".$style['poyas']."','".$item_view['poyas']."');\n
			</script>
		</td>
		<td width=140 height=220 valign=top>
			<div style=\"position:relative; z-index:0; width:140px; height:220px;\">";
			if ($myinfo["battle"]!=0 && $myinfo["login"]==$_SESSION["login"])
			{
				$pr=mysql_query("SELECT id, pr_name, name, pr_cur_uses, hited FROM person_on LEFT JOIN priem on priem.type=person_on.pr_name WHERE id_person=".$myinfo["id"]." and battle_id=".$myinfo["battle"]." and person_on.pr_active=2 and pr_cur_uses>0");
				if (mysql_num_rows($pr))
				{
					echo "<div style=\"position:absolute; left:0px; top:0px; width:120px; height:220px; z-index:2\">";	
					while ($pr_s=mysql_fetch_Array($pr))
					{	
						if ($pr_s["pr_name"]=="xlebnut")
						{
							$add_xleb_sila[7]=10;
							$add_xleb_sila[8]=13;
							$add_xleb_sila[9]=14;
							...
							$add_xleb_sila[12]=15;
							$add_xleb_sila[13]=15;
							$add_xleb=$add_xleb_sila[$myinfo["level"]];
							$myinfo["sila"]=$myinfo["sila"]+$add_xleb;
							$str_xl="Сила + ".$add_xleb;
						}
						echo "<span style=\"position:relative;  width:34px; height:34px;\"><IMG src='img/priem/misc/".(int)$pr_s["id"].".gif' alt='Прием: ".$pr_s["name"].($pr_s["hited"]>0?" (Осталось: ".$pr_s["hited"]." урон)":"")."\n$str_xl'>".($pr_s["pr_cur_uses"]>1?"<small style='background-color: #E0E0E0; position: absolute; right: 1; bottom: 1;'><B>x".($pr_s["pr_cur_uses"]-1)."</B></small>":"")."</span> ";
					}
					echo "</div>";
				}
			}
			...
			{
				echo "<div style=\"position:absolute; left:95px; top:149px; z-index:2\"><img src='img/".$myinfo["zver_obraz"]."' width=45 alt='".$myinfo["zver_name"]." [".$myinfo["zver_level"]."]'></div>";
			}
			$inff="<b>Информация о ".$myinfo["login"]." [".$myinfo["level"]."]</b><br>• Сила: ".($myinfo["zver_on"]==0?$myinfo["sila"]:(($myinfo["zver_type"]=="bear" && $myinfo["zver_level"]>0)?($myinfo["sila"]+$myinfo["zver_level"])." [".$myinfo["sila"]."+".($myinfo["zver_level"])."]":$myinfo["sila"]))."<br>• Ловкость: ".($myinfo["zver_on"]==0?$myinfo["lovkost"]:(($myinfo["zver_type"]=="cheetah" && $myinfo["zver_level"]>0)?($myinfo["lovkost"]+$myinfo["zver_level"])." [".$myinfo["lovkost"]."+".($myinfo["zver_level"])."]":$myinfo["lovkost"]))."<br>• Удача: ".($myinfo["zver_on"]==0?$myinfo["udacha"]:(($myinfo["zver_type"]=="wolf" && $myinfo["zver_level"]>0)?($myinfo["udacha"]+$myinfo["zver_level"])." [".$myinfo["udacha"]."+".($myinfo["zver_level"])."]":$myinfo["udacha"]))."<br>• Выносливость: ".$myinfo["power"]."<br>";
			if($myinfo["level"])
			{
				$inff.="• Интеллект: ".($myinfo["zver_on"]==0?$myinfo["intellekt"]:(($myinfo["zver_type"]=="snake" && $myinfo["zver_level"]>0)?($myinfo["intellekt"]+$myinfo["zver_level"])." [".$myinfo["intellekt"]."+".($myinfo["zver_level"])."]":$myinfo["intellekt"]))."<br>";
				$inff.="• Восприятие: ".$myinfo["vospriyatie"]."<br>";
			}
			if($myinfo["level"]>9 || $myinfo["duxovnost"]>0)
			{
		...
		<td width=60 height=220 valign=top>
			<script>";
				...
				echo "
				view_item('items/".$item['amulet']."',60,20,'".$descr['amulet']."','".$style['amulet']."','".$item_view['amulet']."');\n				
				view_item('items/".$item['ring1']."',20,20,'".$descr['ring1']."','".$style['ring1']."','".$item_view['ring1']."');
				view_item('items/".$item['ring2']."',20,20,'".$descr['ring2']."','".$style['ring2']."','".$item_view['ring2']."');
				view_item('items/".$item['ring3']."',20,20,'".$descr['ring3']."','".$style['ring3']."','".$item_view['ring3']."');
				view_item('items/".$item['perchi']."',60,40,'".$descr['perchi']."','".$style['perchi']."','".$item_view['perchi']."');\n
				view_item('items/".$item['hand_l']."',60,60,'".$descr['hand_l']."','".$style['hand_l']."','".$item_view['hand_l']."');\n
				view_item('items/".$item['pants']."',60,80,'".$descr['pants']."','".$style['pants']."','".$item_view['pants']."');\n
				view_item('items/".$item['boots']."',60,40,'".$descr['boots']."','".$style['boots']."','".$item_view['boots']."');\n
			</script>
		</td>
	</tr>
	</table>";
...
{	
	...
	{	
		...
		else
		{
			...
			$artimg="";
			$ordimg="";
			if($is_personal){$desc.="<img src=img/icon/personal.gif border=0> ";}
			if($art){$desc.="<img src=img/icon/artefakt.gif border=0> ";}
			if($podzemka){$desc.="<img src=img/icon/podzemka.gif border=0> ";}
			if($need_orden){$desc.="<img src=img/orden/$need_orden/0.gif border=0> ";}
			if($is_runa){$desc.="<img src=img/icon/runa.gif border=0> ";}
			$desc.="<b>$name ".($is_modified?" +$is_modified":"")." ($iznos/$iznos_all)</b>";
			if ($dat["add_xp"]>0){$desc.="<br>• Подогнано: ".$dat["add_xp"]." HP";}
			if($gravirovka!=""){$desc.="<br>• Выгравирована надпись: ".$gravirovka;}
			...
			if($max_attack){$desc.="<br>• Макс. урон: ".($max_attack+$is_modified);}
			if ($del_time!="") $desc.="<BR><b>Аренда: до ".(date('d.m.y H:i:s', $del_time))."</b>";
			if ($is_personal)	$desc.="<br><br><font color=brown>Личное оружие персонажа <b>$personal_owner</b></font>";
		}
		$descr[$slot]=$desc;
		$item[$slot]=$dat['img'];
		$item_unwear[$slot]=$itm;
...
{
	$result = mysql_query("SELECT count(*) FROM online WHERE login='".$myinfo["login"]."'");
	$db = mysql_fetch_array($result);
	showPlayer($myinfo);
	echo "<center>";
	...
	{	
		if($db[0])
		{
			...
			if ($myinfo["bs"])echo "<br>координаты [".$myinfo["location"]."]";
		}
		else
		{
			$lv=mysql_query("SELECT MAX(time_stamp) as max FROM report WHERE login='".$myinfo["login"]."' and type='1'");
...
{	
	...
	$now=md5(time());
	Header("Location: main.php?act=inv&razdel=magic&tmp=$now");
}
//------------------------------------------------------------------
...
{
	if ($n==2 && $myinfo['slot'.$slot]) {echo "<a href='main.php?act=setdown_svitok&slot=$slot&id=".$myinfo['slot'.$slot]."'>";}
	if ($n==4 && $myinfo['slot'.$slot]) {echo "<a href='battle.php?act=magic1&slot=$slot&id=".$myinfo['slot'.$slot]."'>";}
	if ($myinfo['slot'.$slot])
	{
		...
		echo "<img src='img/$f[img]' onmouseover=\"slot_view('<b>".($n==2?"Снять ":($n==4?"Использовать ":""))."$f[name]</b><br>Долговечность: $f[iznos]/$f[iznos_max]');\" onmouseout=\"slot_hide();\">";
	}
	else echo "<img src=img/elik/none.gif onmouseover=\"slot_view('Пустой слот заклинания');\" onmouseout=\"slot_hide();\">";
	if ($n==2 || $n==4) {echo "</a>";}
}
/*=====================ПОЛУЧИТЬ ТРАВМУ===========================*/
...
function testUps($user)
{
	$exptable = array(
                        ...
                        "52000000" => array (10,1,1,25000,1,55000000,"Ветеран I степени",600),//level 11
        				...
        				"120000000" => array (15,1,1,50000,1,200000000,"Ветеран II степени",0),//level 12
        				...
        				"1000000000" => array (1, 0,0,10000,0,100000000000,"Ветеран II степени",0),
        				"100000000000" => array (15,1,1,50000,1,1000000000000,"Ветеран II степени",0),//level 13
        				"1000000000000" => array (15,1,1,50000,1,6000000000000,"Ветеран II степени",0),//level 14
        				"6000000000000" => array (15,1,1,50000,1,10000000000000,"Ветеран II степени",0),//level 15
        				"10000000000000" => array (15,1,1,50000,1,14000000000000,"Ветеран II степени",0),//level 16
				);
				/*   ups,umenie,power,money, level, next_up, status,mass*/
	if ($user['exp'] >= $user['next_up'])
	{
		...
		{
			say("toall_news","Воин <b>".$user["login"]."</b> достиг уровня <b>".($user['level']+1)."</b>!",$user["login"]);
...
function testZverUp($zver,$owner)
{
	...
	say($owner,"<b>".$zver["name"]."</b> достиг уровня <b>".$cur_level."</b>!",$owner);
	$now=md5(time());
	Header("Location: main.php?act=none&tmp=$now");
...
/*======================TestBattle===============================*/
function testBattle($who)
{
	$query=mysql_query("SELECT battle FROM users WHERE login='".$who."'");
	$data=mysql_fetch_array($query);
	if($data["battle"]!=0)
	{
		$now=md5(time());
		Header("Location: battle.php?tmp=$now");
	}
}
/*=======================================================================*/
    ...
    else if($is_krit)
    {
		...
		{	
			if (in_Array("xlebnut",$at_priem) && $ATTACK_DATA["hp"]>0 && $DEFEND_DATA["level"]>=7)
			{
				...
				if ($hp_add>$max_add[$DEFEND_DATA["level"]])$hp_add=$max_add[$DEFEND_DATA["level"]];
...
function win($team,$battle)
{
	...
	$bot_battle=array(55,10,80,15,82,33,88,89);
	...
	while($DATA = mysql_fetch_array($T))
	{
		...
		if ($WINNER_DATA["zver_on"] && $WINNER_DATA["zver_level"]<$WINNER_DATA["level"] && $WINNER_DATA["zver_level"]<10)
		{
			...
			talk($WINNER_DATA["login"],$ms,$WINNER_DATA);
		}	
		if(in_array($zay_type,$boylar))
		{
			...
			#talk($WINNER_DATA["login"],"Вы нашли <b>Снежок</b>",$WINNER_DATA);
			...
			#mysql_Query("INSERT INTO inv (owner, object_id, object_type, object_razdel, gift, gift_author, iznos, iznos_max, term) VALUES('".$WINNER_DATA['login']."', '262', 'scroll', 'magic', '1', 'WWW.MEYDAN.AZ', '0', '1', '".(time()+30*24*3600)."');");
			#talk($WINNER_DATA["login"],"Вы нашли 2 <b>Праздничные яйца</b>",$WINNER_DATA);
			...
			talk($WINNER_DATA["login"],"Вы нашли <b>Жетон Падшего Бойца</b>",$WINNER_DATA);
		}
		...
		if ($zay_type==89)
		{
			...
			switch ($WINNER_DATA['room'])
			{
				case "novruz_floor":$etaj=1;break;
				case "novruz_floor2":$etaj=2;break;
			}
			if ($WINNER_DATA['fwd'])
			{
				$gr_id=mysql_fetch_array(mysql_query("SELECT group_id FROM labirint_login WHERE player='".$WINNER_DATA['login']."'"));
				mysql_query("INSERT INTO labirint_setting VALUES (null,'".$WINNER_DATA['fwd']."','".$WINNER_DATA['login']."','".$gr_id['group_id']."','".$etaj."','bot','')");
				$count_led=rand(1,2);
				if ($count_led>0)
				{	
					for ($kk=1;$kk<=$count_led;$kk++)
					{
						$item_led=rand(151,155);
						mysql_query("INSERT INTO labirint_setting VALUES (null,'".$WINNER_DATA['fwd']."','".$WINNER_DATA['login']."','".$gr_id['group_id']."','".$etaj."','items','".$item_led."')");
					}
				}
			}
		}
        ...
        else if ($zay_type==55){$w_exp=$w_exp/4;$monstr_win=1;}
		...
		if($zay_type==1 || $zay_type==100)
		{
			mysql_query("UPDATE daily_kwest SET taked=taked+1 WHERE user_id='".$WINNER_DATA['id']."' and kwest_id=2");
		}
		if($zay_type==4 || $zay_type==101)
		{
			mysql_query("UPDATE daily_kwest SET taked=taked+1 WHERE user_id='".$WINNER_DATA['id']."' and kwest_id=3");
		}
		if($zay_type==5 || $zay_type==102 || $zay_type==7)
		{
			mysql_query("UPDATE daily_kwest SET taked=taked+1 WHERE user_id='".$WINNER_DATA['id']."' and kwest_id=4");
		}
		//------------------------------------------
		mysql_query("UPDATE daily_kwest SET taked=taked+$w_exp WHERE user_id='".$WINNER_DATA['id']."' and kwest_id=1");
		...
		talk($player,$wintext,$WINNER_DATA);
   		//---------хп при победе--------------
	...
	if ($count_winners>20)
	{
		talk("toall","Бой закончен. Благодаря величайшим воинам ".$winners."! Победа за <i>".$all_winners."</i>","");
		$co_arr=count($win_array);
		for ($i=0;$i<$co_arr;$i++)
		{	
			$user=$win_array[$i];
			mysql_query("UPDATE users SET money=money+101 WHERE login='".$user."'");
			say($user,"Вы нашли <b>101.00 Зл.</b> у убитого бойца.",$user);
		}
	}
	...
	if ($zay_type==23) 
	{
		...
		if ($art_winner && $art_winner!="Исчадие Хаоса")
		{
			...
			if ($max_hp_winner && $max_hp_winner!=$art_winner)
			{
				...
				$str_art_winner="и <b>$max_hp_winner</b> (за максимальный опыт)";
			}
...
function goBattle($who)
{
    ...
    if ($DATA)
    {
	    ...
	    Header("Location: battle.php?tmp=$time");
		//die();
	}
}
//============================================================================//
...
function chance($param)
{
    ...
    for($i=$param+1;$i<=100;$i++)
    {
		$chance_suc[$i]="empty";
    }
    $chance_numer=rand(0,100);
    $is_chance=false;
    for($i=0;$i<=100;$i++)
    {
		if($chance_numer==$chance_suc[$i])
		{
        	$is_chance=true;
		}
	}
...
//---------------------------------------------------------------------------
function showHPbattle($myinfo)
{
	$shut=($myinfo['shut']-time()>0)?$shut:0;
	echo "
	<table valign='bottom' border=0 cellpadding=0 cellspacing=0 width=260>
	<tr valign='center'>
		<td align='center' valign='bottom'>
			<script>usr('".$myinfo["login"]."', '".$myinfo["id"]."', '".$myinfo["level"]."', '".$myinfo["dealer"]."', '".$myinfo["orden"]."', '".$myinfo["admin_level"]."', '".$myinfo["clan_short"]."', '".$myinfo["clan"]."','".$shut."','".$myinfo["travm"]."')</script>
		</td>
	</tr>
	</table>
	<script>show(".$myinfo["hp"].",".$myinfo["hp_all"].");</script>";
	if($myinfo["level"] && $myinfo["mana_all"])
	{
		echo "<script>showMN(".$myinfo["mana"].",".$myinfo["mana_all"].");</script>";
	}
}
/*===============================================================*/
function showHPBot($BOT_D)
{
	echo "
	<table border=0 cellpadding=0 cellspacing=0 width=100%>
	<tr>
		<td align=center><b>".$BOT_D["bot_name"]."</b></td>
	</tr>
	</table>
	<script>show(".$BOT_D["hp"].",".$BOT_D["hp_all"].");</script>";
}
/*===============================================================*/
?>