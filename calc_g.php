...
############################################################
$k=mysql_fetch_array(mysql_query("SELECT is_modified, add_rej, add_drob, add_kol, add_rub, art, proboy, two_hand FROM `inv` WHERE id=$attack_weapon"));
...
//---------------------------------------------------------
/*if ($DEFEND_DATA["login"]=="OBITEL")
{
	say($DEFEND_DATA["login"],$DEFEND_DATA["login"]."-".$DEFEND_DATA["exp"]." [uv=".$uv."] [veruv2=".$veruv2."] [is_uvorot=".$is_uvorot."] [Увертывания ".$DEFEND_DATA["login"].": $defend_uvorot] [Пр. увертывания ".$ATTACK_DATA["login"].": $attack_antiuvorot]",$DEFEND_DATA);
	say($DEFEND_DATA["login"],$ATTACK_DATA["login"]."-".$ATTACK_DATA["exp"]." [crit=".$crit."] [verkr2=".$verkr2."] [is_krit=".$is_krit."] [Крит ".$ATTACK_DATA["login"].": $attack_krit] [Пр. Крит ".$DEFEND_DATA["login"].": $defend_antikrit]",$DEFEND_DATA);
}*/
#if ($DEFEND_DATA["login"]=="bor")say($DEFEND_DATA["login"],$DEFEND_DATA["login"]."-".$ATTACK_DATA["login"]."-".voin_type($DEFEND_DATA)."-".voin_type($ATTACK_DATA)."-".$P_HDATA["type"],$DEFEND_DATA);
