...
if($buy)
{
	$query=mysql_query("SELECT * FROM paltar WHERE id='".$item."' and mountown>0 and podzemka=1");
...
<td width=180 valign=top>
	<table width="100%" cellspacing="1" cellpadding="3" class="g"  height=20>
		<tr bgcolor=#212120><td align="center"><B style='color:#ffffff'>Отделы магазина</b></td></tr>
	</table>
	<br>
	<table width="100%" cellspacing="1" cellpadding="3" class="g">
        <tr bgcolor="#cdcdcd">
            <td>
				<INPUT TYPE=button style="width:100%;" onclick="location.href='main.php?act=go&level=katakomba'" value="Выход" class="newbut">	
            </td>
        </tr>
	</table>
	<br>
...
<td valign=top width=100%>
	...
	<B style='color:#ffffff'>Ассортимент товаров отдела "<?=$otdels[$otdel];?>" </B>
	</td></tr>
	<?
		#if (!$db["adminsite"])die("ЗАКРЫТ НА РЕМОНТ!!");
		$seek=mysql_query("SELECT * FROM paltar WHERE object='".$otdel."' and mountown>0 and podzemka=1 ORDER BY min_level ASC,price ASC");
