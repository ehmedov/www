<?
echo "<link rel=stylesheet type='text/css' href='smith.css'>";
...
{	
	...
	$_SESSION['otdel_pod']=$otdel;
}
else 
{
	if (isset($_SESSION['otdel_pod']))
	{
		$otdel=$_SESSION['otdel_pod'];
	}
}
//----------------------------------------------------------------------------------------
if($buy)
{
	$query=mysql_query("SELECT * FROM paltar WHERE id='".$item."' and mountown>0 and art=0 and podzemka=0 and (min_level>3 and min_level<8) ORDER BY min_level ASC,price ASC");
	...
	{
		if ($db["naqrada"]<$buy_item["price"])
		{
			$msg="У Вас нет достаточной <b>Наград</b> для покупки этой вещи!";
		}
		else
		{
			...
			mysql_query("UPDATE users,paltar SET users.naqrada=users.naqrada-".$buy_item["price"].", paltar.".$city."=paltar.".$city."-1 WHERE users.login='".$login."' and paltar.id='".$item."'");	
			$db["naqrada"]=$db["naqrada"]-$buy_item["price"];
			$msg="Вы удачно купили <b>&laquo;".$buy_item["name"]."&raquo;</b> за <b>".$buy_item["price"]." Ед.</b>";
			$name2=$buy_item["name"]." (".$buy_item["price"]." Ед.)";
			history($login,'Купил',$name2,$db["remote_ip"],'Лавка Героев');
		}
		$otdel=$buy;
	}
}
...
<td width=180 valign=top>
	<table width="100%" cellspacing="1" cellpadding="3" class="g"  height=20>
		<tr bgcolor=#212120><td align="center"><B style='color:#ffffff'>&#1042;&#1099;&#1089;&#1096;&#1099;&#1081; &#1047;&#1077;&#1084;&#1085;&#1086;&#1081; &#1059;&#1076;&#1072;&#1088;</b></td></tr>
	</table>
	<br>
	<table width="100%" cellspacing="1" cellpadding="3" class="g">
        <tr bgcolor="#cdcdcd">
            <td>
				<INPUT TYPE=button style="width:100%;" onclick="location.href='main.php?act=go&level=merlin'" value="Выход" class="newbut">	
            </td>
        </tr>
	</table>
	<br>
...
<td valign=top width=100%>
	<TABLE width=100% cellspacing=1 cellpadding=3 bgcolor=212120>
	<tr height=20><td valign=center align=center colspan=2>
	<B style='color:#ff0000'>Ассортимент товаров отдела "<?=$otdels[$otdel];?>" </B>
	</td></tr>
	<?
		#if (!$db["adminsite"])die("ЗАКРЫТ НА РЕМОНТ!!");
		$seek=mysql_query("SELECT * FROM paltar WHERE object='".$otdel."' and mountown>0 and art=0 and podzemka=0 and (min_level>3 and min_level<8) ORDER BY min_level ASC,price ASC");
		while ($res=mysql_fetch_array($seek))
		{
			$n=(!$n);
			echo '<tr bgcolor='.($n?'#C7C7C7':'#D5D5D5').'>
						<td valign=center align=center width=180 nowrap><img src="img/items/'.$res['img'].'"><br>';
						echo '<a href="?buy='.$otdel.'&item='.$res["id"].'">Купить за '.sprintf ("%01.2f", $res["price"]).' Ед.</a><br>';
						echo '</td><td>';
						$db["money"]=$db["naqrada"];
						show_item($db,$res);
