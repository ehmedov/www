...
if($_GET['action']=='attack' && count($Bot_Array[$_GET['id']]) && !$db["zayavka"] && $db["hp"]>0)
{
	...
	if (mysql_num_rows($sel_battle_bot))
	{
		...
		$date = date("H:i");
		$att="<span class=date2>$date</span> <script>drwfl('".$db['login']."','".$db['id']."','".$db['level']."','".$db['dealer']."','".$db['orden']."','".$db['admin_level']."','".$db['clan_short']."','".$db['clan']."');</script> вмешался в поединок!<hr>";		battle_log($battle_id, $att);
...
if($_GET['action']=="get_zakalka" && $step1['fwd']=="2x3")
{
	...
	else $msg= "Для работы необходимо убить всех Плохой Санта вокруг Проруби";
}
//------------------------------------------------------------------
# Декорации
...
<td width=100% valign=top>";
	...
	WHILE ($players=mysql_fetch_Array($pl_sql))
	{
		...
		<td>".($players["leader"]?"<img src='img/led/misc/lid.gif' alt='Лидер группы'>":"").(($players["leader"] && $players["login"]==$login)?"&nbsp;<A href='#' onclick=\"findlogin( 'Выберите персонажа которого хотите выгнать','?act=none', 'kill_member' )\"><IMG alt='Выгнать' src='img/led/misc/kill.gif'></A>&nbsp;<A href='#' onclick=\"findlogin( 'Выберите персонажа которому хотите передать лидерство','?act=none', 'change_leader')\"><IMG alt='Новый Лидер' src='img/led/misc/c_lid.gif'></A>&nbsp;":"")."</td></tr>";
...
if($step1['fwd']=="2x3"){echo"<div style=\"position:absolute; top:10px; left:10px; z-index:100; cursor:hand;\" onclick=\"document.location.href='?action=get_zakalka'\"><img src=\"img/led/misc/zakalka.gif\" title='Прорубь\nНажавшему приз: \"Закалка\" дает (Выносливость*10) на 6 ч.\nДля работы необходимо убить всех Плохой Санта вокруг Проруби.'></div>";}
...
if(in_array($step1["fwd"],$sunduk_Array))
{
	echo"\n<form name='myform' method=POST action='main.php?act=none'>
		<div id=\"m1_4\" style=\"Z-INDEX:10; LEFT:10px; POSITION:absolute; TOP:10px;\">
			<img src='img/led/misc/sunduk1.gif' style='border:0; CURSOR:hand;' onclick=\"if (confirm('Открыть сундук?')) document.myform.submit(); else this.form.action=''; \">
...
{
	echo"<div style=\"Z-INDEX:10; LEFT:10px; POSITION:absolute; TOP:10px; \" title=\"Фонтан жизни\nВосстанавливает полный уровень жизни Вашего персонажа\" onclick=\"take('heal');\"><img src='img/led/misc/".$draw_item_fwd."_fwd1.gif' style='border:0; CURSOR:hand;'></div>";
}
...
if($step1['fwd']) 
{
	echo "<a href='?action=forward' onclick='return check_time();'><img src='img/led/up.gif' style=\"position:absolute; top:12px; left:48px; z-index:0;\" border='0' alt='Вперёд'></a>";
}
else echo "<img src='img/led/noway.gif' style=\"position:absolute; top:14px; left:46px; z-index:0;\" border='0' alt='Нельзя'>";
echo "<a href='?action=rotateleft' onclick='return check_time();'><img src='img/led/left.gif' style=\"position:absolute; top:40px; left:12px; z-index:0;\" border='0' alt='Повернуться влево'></a>";
echo "<a href='?action=rotateright' onclick='return check_time();'><img src='img/led/right.gif' style=\"position:absolute; top:40px; left:88px; z-index:0;\" border='0' alt='Повернуться вправо'></a>";
