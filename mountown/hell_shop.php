...
//***************************************************************************
$item_Array_str="('484', '485', '486', '487', '528', '529', '530', '531', '533', '534', '535', '536')";
$item_array=array();

$item_array[484]=array(array("item"=>141,"count"=>5),array("item"=>140,"count"=>3));
$item_array[485]=array(array("item"=>141,"count"=>5),array("item"=>140,"count"=>3));
$item_array[486]=array(array("item"=>141,"count"=>5),array("item"=>140,"count"=>3));
$item_array[487]=array(array("item"=>141,"count"=>5),array("item"=>140,"count"=>3));

$item_array[528]=array(array("item"=>141,"count"=>10),array("item"=>140,"count"=>5));
$item_array[529]=array(array("item"=>141,"count"=>10),array("item"=>140,"count"=>5));
$item_array[530]=array(array("item"=>141,"count"=>10),array("item"=>140,"count"=>5));
$item_array[531]=array(array("item"=>141,"count"=>10),array("item"=>140,"count"=>5));

$item_array[533]=array(array("item"=>141,"count"=>15),array("item"=>140,"count"=>7));
$item_array[534]=array(array("item"=>141,"count"=>15),array("item"=>140,"count"=>7));
$item_array[535]=array(array("item"=>141,"count"=>15),array("item"=>140,"count"=>7));
$item_array[536]=array(array("item"=>141,"count"=>15),array("item"=>140,"count"=>7));

$magic_Array_str="('250','251','252','253','254','255','256','257')";

$magic_Array=array();
$magic_Array[250]=array(array("item"=>140,"count"=>5));
$magic_Array[251]=array(array("item"=>140,"count"=>5));
$magic_Array[252]=array(array("item"=>140,"count"=>5));
$magic_Array[253]=array(array("item"=>140,"count"=>5));
$magic_Array[254]=array(array("item"=>140,"count"=>5));
$magic_Array[255]=array(array("item"=>140,"count"=>5));
$magic_Array[256]=array(array("item"=>140,"count"=>5));
$magic_Array[257]=array(array("item"=>140,"count"=>5));
//***************************************************************************
if ($_GET["buy_magic"])
{
	$_POST['shop']=true;
...
{	
	...
	else
	{
		...
		if (!in_Array(0,$a))
		{
			...
			history($login,'Купил',$buy_item["name"],$db["remote_ip"],'Таинственный Маяк');
		}
	}
}
//************************************************************
echo "<h3>Оружейная Комната</h3>";
...
<td align=right nowrap>
	<INPUT TYPE=button value=\"Обновить\" onClick=\"location.href='main.php?act=none'\">
 	<INPUT TYPE=button value=\"Вернуться\" onClick=\"location.href='main.php?act=go&level=mayak';\">
</td>
</tr>
</table><hr>
				...
				if ($_GET['otdel']=="obj")
				{
					...
					while ($res=mysql_fetch_array($seek))
					{
						...
						{	
							...
							{
								$query=mysql_fetch_Array(mysql_query("SELECT img, name FROM wood WHERE id=".$currentValues["item"]));
								$str.="<span style=\"position:relative;  width:30px; height:40px;\"><img src='img/".$query["img"]."' title='".$query["name"]."'><small style='background-color: #E0E0E0; position: absolute; right: 0; bottom: 0;'><B>".$currentValues["count"]." шт.</B></small></span>&nbsp;&nbsp;";
							}
							echo "<br><b>Требуется предмет</b><br>".$str;
						}
						echo '</td>
						<td>';
					}
					...
					while ($dat = mysql_fetch_array($seek))
					{
						...
						<TR  bgcolor=".($n?'#D5D5D5':'#C7C7C7').">
							<TD valign=center align=center width=200>
								...
								<a href='?buy_magic=".$dat["id"]."'>Купить</a>";
							echo "</td>
						<td valign=top>
						...
						if (count($magic_Array[$dat["id"]]))
						{	
							foreach ($magic_Array[$dat["id"]] as $currentValues)
							{
								$query=mysql_fetch_Array(mysql_query("SELECT img, name FROM wood WHERE id=".$currentValues["item"]));
								$str.="<span style=\"position:relative;  width:30px; height:40px;\"><img src='img/".$query["img"]."' title='".$query["name"]."'><small style='background-color: #E0E0E0; position: absolute; right: 0; bottom: 0;'><B>".$currentValues["count"]." шт.</B></small></span>&nbsp;&nbsp;";
							}
