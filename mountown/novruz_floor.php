...
$etaj=1;
$db["room"]="novruz_floor";
include("modules/novruz_mod.php");
//----------------------------------------------
$sql_=mysql_fetch_array(mysql_query("SELECT * FROM labirint_login WHERE player='".$login."'"));
$group_id=$sql_["group_id"];
$leader=$sql_["leader"];
if (!$group_id)
{
	Header("Location: main.php?act=go&level=novruz_go&tmp=$now");
	die();
}	
//----------------------------------------------
$bot_d=mysql_query("SELECT cord FROM labirint_setting WHERE group_id='".$group_id."' and type='bot' and etaj=$etaj");
...
if ($_GET["exit"])
{
	...
	$all_team=mysql_fetch_array(mysql_query("SELECT count(*) as co  FROM labirint_login WHERE group_id=$group_id"));
	if (!$all_team["co"])
	{
		mysql_query("DELETE FROM labirint_group WHERE id='".$group_id."'");
		mysql_query("DELETE FROM labirint_setting WHERE group_id='".$group_id."'");
	}
	Header("Location: main.php?act=go&level=novruz_go&tmp=$now");
	die();
}
...
if($_GET['action']=='attack' && count($Bot_Array[$_GET['id']]) && !$db["zayavka"] && $db["hp"]>0)
{
	...
	else
	{
	    ...
	    mysql_query("INSERT INTO zayavka(status,type,timeout,creator) VALUES('3','89','3','".$mine_id."')");
	    mysql_query("INSERT INTO teams(player, team, ip, battle_id) VALUES('".$login."', '1', '".$ip."', '".$mine_id."')");
		mysql_query("INSERT INTO battles(type, creator_id, lasthit) VALUES('89', '".$mine_id."', '".$timeout."')");
...
if ($_POST["kill_member"] && $leader)
{
	...
	$sel_kill=mysql_fetch_array(mysql_query("SELECT * FROM labirint_login WHERE player='".$kill_member."' and group_id=".$group_id));
	if ($sel_kill && $sel_kill["player"]!=$login)
	{
		mysql_query("DELETE FROM labirint WHERE user_id='".$kill_member."'");
		mysql_query("DELETE FROM labirint_login WHERE player='".$kill_member."'");
...
if ($_POST["change_leader"] && $leader)
{
	...
	$sel_kill=mysql_fetch_array(mysql_query("SELECT * FROM labirint_login WHERE player='".$change_leader."' and group_id=".$group_id));
	if ($sel_kill && $sel_kill["player"]!=$login)
	{
		mysql_query("UPDATE labirint_login SET leader=1 WHERE player='".$change_leader."'");
		mysql_query("UPDATE labirint_login SET leader=0 WHERE player='".$login."'");
		...
		if($step1['fwd'] && (!in_array($step1["fwd"],$sunduk_Array)) && (!count($Bot_Array[$step1['fwd']])))
		{
			if ($step1['fwd']=="4x3" && $my_vector==180)
			{
				...
				{	
					...
					else $msg="Не хватает: Ключ от Лабиринта";
				}
				else
				{
					$my_cord=$step1['fwd'];
...
{
	$get=(int)$_GET["get"];
	$ww=mysql_fetch_array(mysql_query("SELECT item_id, name FROM labirint_setting LEFT JOIN wood on wood.id=labirint_setting.item_id WHERE labirint_setting.group_id='".$group_id."' and labirint_setting.cord='".$my_cord."' and labirint_setting.ids=$get and etaj=$etaj and type='items'"));
	if (!$ww)
	{
		$msg="Кто-то быстрее...";
	}
	else
	{
		...
		mysql_query("DELETE FROM labirint_setting WHERE ids='".$get."'");
...
if ($_POST["id_sund"] && in_array($_POST["id_sund"],$sunduk_Array) && $step1['fwd']==$_POST["id_sund"])
{
	...
	$s_id=mysql_query("SELECT * FROM labirint_setting WHERE group_id='".$group_id."' and type='sunduk' and etaj=$etaj and cord='$id_sund'");
	if (!mysql_fetch_array($s_id))
	{
		foreach ($eliks[$id_sund] as $table => $take_array) 
		{
			...
			if ($eliksir)
			{
				...
				mysql_query("INSERT INTO labirint_setting VALUES (0,'".$id_sund."','".$login."','".$group_id."','$etaj','sunduk','')");
...
{
	$have_bots=mysql_fetch_Array(mysql_query("SELECT count(*) FROM labirint_setting WHERE group_id='".$group_id."' and type='bot' and etaj=$etaj and cord in ('3x2','3x4','1x4','1x2')"));
	if($have_bots[0]>=4)
	{
		...
		if (!$have_elik)
		{
			...
			$msg="Вы удачно прокастовали заклинание...";
		}
		else
		{
			$msg="Вы уже использовал это заклятие...";
		}		
	}
	else $msg= "Для работы необходимо убить всех Ботов вокруг Проруби";
}
//------------------------------------------------------------------
# Декорации
...
<div id=hint3></div>
<h3>Пещера Праздничная (Этаж 1)</h3>
...
<td width=100% valign=top>";
	...
	$pl_sql=mysql_query("SELECT users.id,users.login,level,dealer,orden,admin_level,clan_short,clan,hp,hp_all,labirint.location,labirint_login.leader FROM users LEFT JOIN labirint_login on group_id='".$group_id."' LEFT JOIN labirint_group on labirint_group.id='".$group_id."' LEFT JOIN labirint on labirint.user_id=labirint_login.player WHERE labirint_login.player=users.login and labirint.etaj=1");
	WHILE ($players=mysql_fetch_Array($pl_sql))
	{
		...
		<td>".($players["leader"]?"<img src='img/novruz/misc/lid.gif' alt='Лидер группы'>":"").(($players["leader"] && $players["login"]==$login)?"&nbsp;<A href='#' onclick=\"findlogin( 'Выберите персонажа которого хотите выгнать','?act=none', 'kill_member' )\"><IMG alt='Выгнать' src='img/novruz/misc/kill.gif'></A>&nbsp;<A href='#' onclick=\"findlogin( 'Выберите персонажа которому хотите передать лидерство','?act=none', 'change_leader')\"><IMG alt='Новый Лидер' src='img/novruz/misc/c_lid.gif'></A>&nbsp;":"")."</td></tr>";
		if ($login!=$players["login"])$users[]=array(login=>$players["login"],coord=>$players["location"]);
	}
	mysql_free_result($pl_sql);
	echo "</table><br>";
	echo "<FONT COLOR=red>$msg</FONT>";
	$predmet=mysql_query("SELECT ids,name,img FROM labirint_setting LEFT JOIN wood on wood.id=labirint_setting.item_id WHERE labirint_setting.group_id='".$group_id."' and labirint_setting.cord='".$my_cord."' and labirint_setting.etaj=1 and type='items'");
...
echo "\n\n <!--1 слой-->\n";
if(!$step1["left"])	{echo"<div style=\"position:absolute; top:10px; left:10px; z-index:7;\"><img src=\"img/novruz/1_left_wall.gif\" /></div>";}
if(!$step1["fwd"])	{echo"<div style=\"position:absolute; top:10px; left:10px; z-index:7;\"><img src=\"img/novruz/1_front_wall.gif\" /></div>";}
if(!$step1["right"]){echo"<div style=\"position:absolute; top:10px; right:0px; z-index:7;\"><img src=\"img/novruz/1_right_wall.gif\" /></div>";}

if($step1['fwd']=="4x3" && ($my_vector==180 || $my_vector==0)){echo"<div style=\"position:absolute; top:10px; left:10px; z-index:100;\"><img src=\"img/novruz/misc/gate.gif\"></div>";}
if($step1['fwd']=="2x3"){echo"<div style=\"position:absolute; top:10px; left:10px; z-index:100; cursor:hand;\" onclick=\"document.location.href='?action=get_zakalka'\"><img src=\"img/novruz/misc/zakalka.gif\" title='Прорубь\nНажавшему приз: \"Закалка\" дает (Выносливость*30) на 6 ч.\nДля работы необходимо убить всех Ботов вокруг Проруби.'></div>";}
...
if ($users!="")
{
	foreach ($users as $currentValue) 
	{
		...
		{
			echo"<div style=\"Z-INDEX:12; LEFT:130px; POSITION:absolute; TOP:40px;\"><img src=\"img/novruz/shadow.gif\" alt=\"".$currentValue["login"]."\" style=\"CURSOR:hand\"></div>";
		}
	}
}
if(in_array($step1["fwd"],$sunduk_Array))
{
	echo"\n<form name='myform' method=POST action='main.php?act=none'>
		<div id=\"m1_4\" style=\"Z-INDEX:10; LEFT:10px; POSITION:absolute; TOP:10px;\">
			<img src='img/novruz/misc/sunduk1.gif' style='border:0; CURSOR:hand;' onclick=\"if (confirm('Открыть сундук?')) document.myform.submit(); else this.form.action=''; \">
...
if($draw_item_left)
{
	echo"<div style=\"Z-INDEX:6; LEFT:10px; POSITION:absolute; TOP:10px; \"><img src='img/novruz/misc/".$draw_item_left."_l1.gif' style='border:0; CURSOR:hand;'></div>";
}
if($draw_item_right)
{
	echo"<div style=\"Z-INDEX:10; LEFT:10px; POSITION:absolute; TOP:10px; \"><img src='img/novruz/misc/".$draw_item_right."_r1.gif' style='border:0; CURSOR:hand;'></div>";
}
if($draw_item_fwd)
{
	echo"<div style=\"Z-INDEX:10; LEFT:10px; POSITION:absolute; TOP:10px; \"><img src='img/novruz/misc/".$draw_item_fwd."_fwd1.gif' style='border:0; CURSOR:hand;'></div>";
}
echo"\n\n<!--2 слой-->\n";
if(!$step2["left"])	{echo "<div style=\"position:absolute; top:10px; left:10px; z-index:5;\"><img src=\"img/novruz/2_left_wall.gif\" /></div>"; }
if(!$step2["fwd"])	{echo "<div style=\"position:absolute; top:10px; left:10px; z-index:5;\"><img src=\"img/novruz/2_front_wall.gif\" /></div>";}
if(!$step2["right"]){echo "<div style=\"position:absolute; top:10px; right:0px; z-index:5;\"><img src=\"img/novruz/2_right_wall.gif\" /></div>";}

if($step2['fwd']=="4x3" && ($my_vector==180 || $my_vector==0)){echo"<div style=\"position:absolute; top:10px; left:10px; z-index:100;\"><img src=\"img/novruz/misc/gate1.gif\"></div>";}
if($step2['fwd']=="2x3" && ($my_vector==180 || $my_vector==0)){echo"<div style=\"position:absolute; top:10px; left:10px; z-index:7;\"><img src=\"img/novruz/misc/zakalka1.gif\"></div>";}
...
if ($users!="")
{
	foreach ($users as $currentValue) 
	{
		...
		{
			echo"<div style=\"Z-INDEX:11; LEFT:150px; POSITION:absolute; TOP:60px;\"><img src=\"img/novruz/shadow.gif\" width=\"70\" alt=\"".$currentValue["login"]."\" style=\"CURSOR:hand\"></div>";
		}
	}
}
if(in_array($step2["fwd"],$sunduk_Array))
{
	echo "\n<div style=\"Z-INDEX:6; LEFT:10px; POSITION:absolute; TOP:10px;\"><img src='img/novruz/misc/sunduk2.gif'></div>";
}

if($draw_item_left1)
{
	echo"<div style=\"Z-INDEX:6; LEFT:10px; POSITION:absolute; TOP:10px; \"><img src='img/novruz/misc/".$draw_item_left1."_l2.gif' style='border:0; CURSOR:hand;'></div>";
}
if($draw_item_right1)
{
	echo"<div style=\"Z-INDEX:10; LEFT:10px; POSITION:absolute; TOP:10px; \"><img src='img/novruz/misc/".$draw_item_right1."_r2.gif' style='border:0; CURSOR:hand;'></div>";
}
if($draw_item_fwd1)
{
	echo"<div style=\"Z-INDEX:10; LEFT:10px; POSITION:absolute; TOP:10px; \"><img src='img/novruz/misc/".$draw_item_fwd1."_fwd2.gif' style='border:0; CURSOR:hand;'></div>";
}

echo"\n\n<!--3 слой-->\n";
if(!$step3["left"]) {echo"<div style=\"position:absolute; top:10px; left:10px; z-index:3;\"><img src=\"img/novruz/3_left_wall.gif\" /></div>";}
if(!$step3["fwd"])	{echo"<div style=\"position:absolute; top:10px; left:10px; z-index:3;\"><img src=\"img/novruz/3_front_wall.gif\" /></div>";}
if(!$step3["right"]){echo"<div style=\"position:absolute; top:10px; right:0px; z-index:3;\"><img src=\"img/novruz/3_right_wall.gif\" /></div>";}

echo"\n\n<!--пол и потолок-->";
echo "<div id=\"l1ce\" style=\"position:absolute; top:10px; left:10px; z-index:0;\"><img src=\"img/novruz/bg.gif\"/></div>";

echo"\n\n<!--4 слой-->";
if(!$step4["left"])	{echo"<div id=\"l4l\" style=\"position:absolute; top:10px; left:10px; z-index:1;\"><img src=\"img/novruz/4_left_wall.gif\" id=\"lv4l\" /></div>";}
if(!$step4["right"]){echo"<div id=\"l4r\" style=\"position:absolute; top:10px; right:0px; z-index:1;\"><img src=\"img/novruz/4_right_wall.gif\" id=\"lv4r\" /></div>";}

echo"<div style=\"position:absolute; bottom:10px; left:120px; z-index:100;\">
<div><img src=\"img/novruz/navigation.gif\"/></div>
<div>";
if($step1['fwd']) 
{
	echo "<a href='?action=forward' onclick='return check_time();'><img src='img/novruz/up.gif' style=\"position:absolute; top:12px; left:48px; z-index:0;\" border='0' alt='Вперёд'></a>";
}
else echo "<img src='img/novruz/noway.gif' style=\"position:absolute; top:14px; left:46px; z-index:0;\" border='0' alt='Нельзя'>";
echo "<a href='?action=rotateleft' onclick='return check_time();'><img src='img/novruz/left.gif' style=\"position:absolute; top:40px; left:12px; z-index:0;\" border='0' alt='Повернуться влево'></a>";
echo "<a href='?action=rotateright' onclick='return check_time();'><img src='img/novruz/right.gif' style=\"position:absolute; top:40px; left:88px; z-index:0;\" border='0' alt='Повернуться вправо'></a>";
