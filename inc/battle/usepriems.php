<?
	...
	if ($is_pr)
	{
		...
		{	
			if ($is_pr["pr_cur_uses"]>0)
			{
				if ($is_pr["pr_wait_for"]) 
				{
					...
					{
						$enable=false;echo "<b style='color:red'>Нельзя использовать: еще идет задержка </b>";
					}
					if ($is_pr['pr_active']!=1 && $is_pr["pr_cur_uses"]<=1) {$enable=false;echo "<b style='color:red'>Нельзя использовать: активен</b>";}
				}
				else
				{
					if ($is_pr['pr_active']!=1 && $is_pr["pr_cur_uses"]<=1) {$enable=false;echo "<b style='color:red'>Нельзя использовать: уже активен</b>";}
				}
			}
			else
			{
				$enable=false;echo "<b style='color:red'>Использовать прием можно 1 раз за бой.</b>";
			}
		}
		else 
		{
			$enable=false;
			echo "<b style='color:red'>Нельзя использовать: не хватает требований</b>";
		}
		...
		if ($enable)
		{
			if ($is_pr["mag"]==1)
			{
				if ($db["bs"]==1)
				{
					 echo "<b style='color:red'>Запрещено использовать приемы мага в турнире</b>";
				}
				else
				{	
					...
					else echo "<b style='color:red'>У Вас недостаточно Маны [мин.".$is_pr["mana"]."MN]</b>";
				}
			}	
			...
			else if ($priem=="restore")
			{
				if ($db['hp']>0)
				{
					...
					else echo "<b style='color:red'>Требует наличие щита</b>";
				}
			}
			else if ($priem=="xlebnut")
			{
				if ($db['hp']>0)
				{
					$phrase_priem  = "<span class=date>$date</span> <span class=$span>$login</span> понял что его спасение это прием <b>Хлебнуть крови</b>.<br> ";
					battle_log($bid, $phrase_priem);
					mysql_query("UPDATE person_on SET pr_active=2,pr_wait_for=12,pr_cur_uses=4 WHERE id_person='".$db["id"]."' and pr_name='".$priem."' and battle_id='".$bid."'");
					mysql_query("UPDATE battle_units SET hit=hit-".(int)$is_pr["hit"].",krit=krit-".(int)$is_pr["krit"].",uvarot=uvarot-".(int)$is_pr["uvarot"].",block=block-".(int)$is_pr["block"].",hp=hp-".(int)$is_pr["hp"].",counter=counter-".(int)$is_pr["all_hit"]." WHERE player='".$login."' and battle_id='".$bid."'");
				}
			}
	...
	{
		echo "<b style='color:red'>Нет такого приема на персонаже</b>";
	}
?>