...
if(!empty($target))
{
	if($db["orden"]==1 && $db["admin_level"]>=2)
	{
		...
		say("toall","Представитель порядка <b>&laquo;".$login."&raquo;</b> использовал$prefix заклятие молчания на персонажа <b>&laquo;".$res['login']."&raquo;</b> на неделю. $reson",$login);
		history($target,"Молчанка на неделю",$reson,$ip,$login);
		history($login,"Молчанка на неделю",$reson,$ip,$target);
