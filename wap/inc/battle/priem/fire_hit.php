...
if($db["battle"] && $db["hp"] && $db["battle_opponent"]!="")
{
	...
	$b_id = $db["battle"];
	$phrase ="";
	$mbot = mysql_query("SELECT users.id,users.power,users.protect_fire,users.protect_mag, bot_temp.hp, bot_temp.hp_all FROM `bot_temp` LEFT JOIN users on users.login=bot_temp.prototype WHERE battle_id='".$b_id."' AND bot_name='".$select_target."'");
