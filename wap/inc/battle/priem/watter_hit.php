...
if($db["battle"] && $db["hp"] && $db["battle_opponent"]!="")
{
	...
	switch ($is_pr["type"]) 
	{
		case "watter8": $damage_min=30;$damage_max=35;break;
		case "watter9": $damage_min=40;$damage_max=43;break;
		case "watter10":$damage_min=50;$damage_max=51;break;
	}
	...
	for($i=0;$i<$n;$i++) 
	{
		...
		$uron=$uron-($victims[$i]["protect_water"]+$effect["protect_water"])-($victims[$i]["protect_mag"]+$effect["add_mg_bron"])/4;
		if ($uron<=0)$uron=3*$db["level"];
		if (in_Array("protwatter",$def_priem))
		{
			$uron=$uron/2;
			mysql_query("UPDATE person_on SET pr_cur_uses=pr_cur_uses-1 WHERE id_person='".$victims[$i]["id"]."' and battle_id=".$b_id." and pr_name='protwatter'");
			$my_res=mysql_fetch_array(mysql_query("SELECT pr_cur_uses FROM person_on WHERE id_person=".$victims[$i]["id"]." and battle_id=".$b_id." and pr_name='protwatter'"));
			if ($my_res["pr_cur_uses"]==1)
			mysql_query("UPDATE person_on SET pr_active=1 WHERE id_person='".$victims[$i]["id"]."' and battle_id=".$b_id." and pr_name='protwatter'");
		}
		...
		$phrase.="<span class=date>$date</span> <span class=$span>".$login."</span> выпустил <span class=magic>".$is_pr["name"]."</span> на <span class=$span2>".$opponenti."</span>, <span class=hitted>-$uron</span> [".$hp_new."/".$victims[$i]["hp_all"]."]<BR>";
		$ret['uron']+=$uron;	
	}
	...
	mysql_query("UPDATE person_on SET pr_active=1,pr_wait_for=".$is_pr["wait"]." WHERE id_person='".$db["id"]."' and pr_name='watter8' and battle_id='".$b_id."'");
	mysql_query("UPDATE person_on SET pr_active=1,pr_wait_for=".$is_pr["wait"]." WHERE id_person='".$db["id"]."' and pr_name='watter9' and battle_id='".$b_id."'");
	mysql_query("UPDATE person_on SET pr_active=1,pr_wait_for=".$is_pr["wait"]." WHERE id_person='".$db["id"]."' and pr_name='watter10' and battle_id='".$b_id."'");
