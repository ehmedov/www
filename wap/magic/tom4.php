...
if ($have_tom_sl11[0])
{
	$have_tom_sl12=mysql_fetch_array(mysql_query("SELECT count(*) FROM slots_priem WHERE user_id=".$db["id"]." and sl_name='sl12'"));
	if ($have_tom_sl12[0])
	{	
	...
	{
		mysql_query("INSERT INTO slots_priem (user_id,sl_name) values (".$db["id"].",'sl12')");
...
{
	$_SESSION["message"]="Изучите Том Страха!";
}
?>