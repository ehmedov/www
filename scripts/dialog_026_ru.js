...
// Заголовок, название скрипта, имя поля с логином
function bank_open(ac_list, ac_def) {
...
opt += '<option value="' + ac[i] + '"' +((ac_def && (ac_def == ac[i]))?' selected':'')+ '>'+ ac[i] + '</option>';
	...
	//s='<table border=0 width=100% cellspacing="0" cellpadding="2"><tr><form action="'+script+'" method=POST name=slform><td colspan=2>'+
//	header + '</TD></TR>' +
//	'<TR><TD width=50% align=right style="padding-left:5">' + opt+ '<br><INPUT style="width: 100%" TYPE="text" NAME="'+name+'" value="'+defaultlogin+'"></TD><TD width=50%><INPUT type=image SRC="#IMGSRC#" WIDTH="27" HEIGHT="20" BORDER=0 ALT="" onclick="slform.'+name+'.value=fixspaces(slform.'+name+'.value);">'+(addon?addon:'')+'</TD></TR></FORM></TABLE>';
//	s = crtmagic(mtype, title, s);
//
//	document.all("hint4").innerHTML = s;
//	document.all("hint4").style.visibility = "visible";
...
var s='<table width="500" border="0" align="center" cellpadding="0" cellspacing="0">'+
	'<tr>'+
	'<td width="100%">'+
